...
public class ArtInfoDao {
    ...
    //删除文章
    public int delArtById(int id) throws SQLException {
        int result=0;
        Connection connection=DBUtils.getConnect();
    ...
    public List<ArticleInfoVo> getList() throws SQLException {
        ...
        String sql = "select a.*,u.username from articleinfo a left join userinfo u on a.uid=u.id";        Connection connection = DBUtils.getConnect();
        PreparedStatement statement = connection.prepareStatement(sql);
        ResultSet resultSet = statement.executeQuery();
        while (resultSet.next()) {
